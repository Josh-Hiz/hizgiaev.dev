...
const isVariant = (v: unknown): v is BackgroundVariant =>
  typeof v === "string" &&
  (backgroundVariants as readonly string[]).includes(v);
...
export function readSettings(): BackgroundSettings {
  ...
  try {
    ...
    return {
      ...
      enabled:
        typeof p.enabled === "boolean" ? p.enabled : DEFAULT_SETTINGS.enabled,
    };
  } catch {
...
function applyToDocument(settings: BackgroundSettings): void {
  ...
  root.style.setProperty(
    "--shader-fallback",
    backgroundFallbacks[settings.variant],
  );
}
...
export function writeSettings(
  ...
  window.dispatchEvent(
    new CustomEvent<BackgroundSettings>(CHANGE_EVENT, { detail: next }),
  );
  return next;
}
...
export function resetSettings(): BackgroundSettings {
  ...
  window.dispatchEvent(
    new CustomEvent<BackgroundSettings>(CHANGE_EVENT, {
...
export function subscribe(
  ...
  const onCustom = (e: Event) =>
    onChange((e as CustomEvent<BackgroundSettings>).detail);
